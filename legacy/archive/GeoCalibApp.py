...
class SolverThread(QThread):
    ...
    def __init__(self, toa_matrix, initial_xyz=None, lambda_param=0.050, 
                 max_iter=10000, eps_limit=1e-10, temp_celsius=26):
        super().__init__()
    ...
    def run(self):
        """Run the RMDU solver in the background thread.
        
        THREAD-SAFETY GUARANTEE: Signal emissions (self.iteration.emit, etc) are
        automatically thread-safe in Qt and will execute their connected slots in
        the main Qt thread. Therefore, slots like update_3d_visualization() will
        always run in the main thread, preventing:
        - QObject::killTimer: Timers cannot be stopped from another thread
        - QObject::setParent: Cannot set parent, new parent is in a different thread
        
        Do NOT access GUI objects or emit timers directly - use signals only.
        """
        try:
            ...
            self.status.emit("Initializing RMDU solver...")
            
            # Convert absolute TOA (in seconds) to distance matrix (in meters)
            # TOA is passed as (NbFiles, NbMics) or (NbMics,)
            # Convert to distance: distance = C * TOA, then transpose to (NbMics, NbFiles) for RMDU
            distance_matrix = (self.C * self.toa_matrix).T
            
            self.status.emit("Running RMDU solver...")
            solver = RMDU(distance_matrix)
            solver.Run(
                lbda=self.lambda_param,
                Xinit=self.initial_xyz,
                itmax=self.max_iter,
                EpsLim=self.eps_limit,
                verbose=0
            )
            
            # Emit iterations as they happen
            # Note: Signal emissions are thread-safe and automatically marshaled to the main thread
            XYZ_iters = solver.X.copy()
            total_iters = XYZ_iters.shape[0]
            
            for i in range(total_iters):
                # Signal emission automatically executes slot in main thread
                self.iteration.emit(XYZ_iters[i])
                progress = int((i + 1) / total_iters * 100)
                self.progress.emit(progress)
            
            # Extract dimensions from solver results
            XYZ_last = XYZ_iters[-1, ...].copy()
            Nm = XYZ_last.shape[0]  # Number of microphone points
            Ns = distance_matrix.shape[0]  # Number of measurement files
            ...
            results = {
                ...
                'Nm': Nm,
                'Ns': Ns,
                'final_error': solver.e[-1] if hasattr(solver, 'e') else None
            }
            
            self.status.emit(f"Solver completed in {len(XYZ_iters)} iterations")
...
class GeoCalibApp(QMainWindow):
    ...
    def plot_final_geometry(self):
        """Plot final geometry in 3D with colors by microphone index and white edges"""
        if self.xyz_final is None:
            QMessageBox.warning(self, "Error", "No geometry data available")
            return
        ...
        if Ns > 0 and xyz.shape[0] > Ns:
             ...
             # Plot Sources (Green, Large)
             for pos in sources:
                mesh = gl.MeshData.sphere(radius=0.1, rows=10, cols=10)
                mesh_item = gl.GLMeshItem(
                    meshData=mesh, smooth=True, drawEdges=True, edgeColor=(1, 1, 1, 1),
                    color=(0, 1, 0, 1)
                )
        ...
        # Plot each point as a mesh sphere with proper coloring
        for i, (pos, color) in enumerate(zip(mics, colors_rgba)):
            # Create a small sphere mesh
            mesh = gl.MeshData.sphere(radius=0.05, rows=10, cols=10)
            
            # Create mesh item with color
            mesh_item = gl.GLMeshItem(
                meshData=mesh,
                smooth=True,
                drawEdges=True,  # Enable edge drawing
                edgeColor=(1, 1, 1, 1),  # White edges
                color=color  # Color by microphone index
            )
        ...
        # Highlight reference mics with larger size if available
        if self.mic_indices is not None:
            for mic_idx in self.mic_indices:
                if mic_idx < len(mics):
                    pos = mics[mic_idx]
                    color = colors_rgba[mic_idx]
                    
                    # Create larger sphere for reference mics
                    mesh = gl.MeshData.sphere(radius=0.08, rows=12, cols=12)
                    mesh_item = gl.GLMeshItem(
                        meshData=mesh,
                        smooth=True,
                        drawEdges=True,
